...
TEST(HardswishF32, Correctness) {
    ...
    hardswish_f32_kernel::run(d_x, d_y, N);
...
TEST(HardswishF32x4, Correctness) {
    const int N = 4096;
    std::vector<float> h_x(N), h_ref(N), h_gpu(N);
    for (int i = 0; i < N; i++) {
        h_x[i] = static_cast<float>(rand()) / RAND_MAX * 20.0f - 10.0f;
    ...
    hardswish_f32x4_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(float), cudaMemcpyDeviceToHost);
...
TEST(HardswishF16, Correctness) {
    const int N = 4096;
    std::vector<half> h_x(N), h_gpu(N);
    std::vector<float> h_ref(N);
    for (int i = 0; i < N; i++) {
        float v = static_cast<float>(rand()) / RAND_MAX * 20.0f - 10.0f;
        h_x[i] = __float2half(v);
        h_ref[i] = ref_hardswish(v);
    }
    half *d_x, *d_y;
    ...
    int block = 256, grid = (N + block - 1) / block;
    hardswish_f16_kernel::run(d_x, d_y, N);
...
TEST(HardswishF16x2, Correctness) {
    const int N = 4096;
    std::vector<half> h_x(N), h_gpu(N);
    std::vector<float> h_ref(N);
    for (int i = 0; i < N; i++) {
    ...
    hardswish_f16x2_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(half), cudaMemcpyDeviceToHost);
...
TEST(HardswishF16x8, Correctness) {
    const int N = 4096;
    std::vector<half> h_x(N), h_gpu(N);
    std::vector<float> h_ref(N);
    for (int i = 0; i < N; i++) {
    ...
    hardswish_f16x8_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(half), cudaMemcpyDeviceToHost);
...
TEST(HardswishF16x8Pack, Correctness) {
    const int N = 4096;
    std::vector<half> h_x(N), h_gpu(N);
    std::vector<float> h_ref(N);
    for (int i = 0; i < N; i++) {
    ...
    int block = 256, grid = (N + 8 * block - 1) / (8 * block);
    hardswish_f16x8_pack_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(half), cudaMemcpyDeviceToHost);
