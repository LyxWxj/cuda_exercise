...
TEST(ReluF32x4, Correctness) {
    ...
    relu_f32x4_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(float), cudaMemcpyDeviceToHost);
...
TEST(ReluF16x2, Correctness) {
    ...
    relu_f16x2_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(half), cudaMemcpyDeviceToHost);
...
TEST(ReluF16x8, Correctness) {
    ...
    relu_f16x8_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(half), cudaMemcpyDeviceToHost);
...
// ---------- relu f16x8 pack ----------
TEST(ReluF16x8Pack, Correctness) {
    const int N = 4096;
    std::vector<half> h_x(N), h_gpu(N);
    std::vector<float> h_ref(N);
    for (int i = 0; i < N; i++) {
    ...
    int block = 256, grid = (N + 8 * block - 1) / (8 * block);
    relu_f16x8_pack_kernel::run(d_x, d_y, N);
    cudaMemcpy(h_gpu.data(), d_y, N * sizeof(half), cudaMemcpyDeviceToHost);
